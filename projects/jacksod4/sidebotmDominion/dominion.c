...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
		case adventurer:
			return(adventurerCard(currentPlayer, state, temphand, &z));
 ...
 *						Adventurer Card
 **********************************************************/
int adventurerCard(int player, struct gameState * state, int * tempHand, int * tHcount){
	int drawnTreasure = 0;
	
	//Continue until 2 treasures have been drawn
	while (drawnTreasure < 2) {
		...
		//Draw a card to the correct players hand
		drawCard(player, state);
		
		//Get the card from the game state
		int cardDrawn = state -> hand[ player ][ state->handCount[player] - 1];

		//If the card was treasure, increment drwan treasure
		if(cardDrawn == copper || cardDrawn == silver || cardDrawn == gold) {
			drawnTreasure += cardDrawn;
			//BUG HERE should be
			//drawnTreasure ++;

		//If not treasure add it to the temp hand
		}else{
	...
	//Discard all cards in temp hand
	while((*tHcount) - 1 >= 0){

		state->discard[player][state->discardCount[player]++]= tempHand[(*tHcount)--];
...
int smithyCard(int player, struct gameState * state, int handPos){
	...
	//Discard the current card (smithy)
	//Bug here (UNCOMMENT BELOW)
...
int villageCard(int player, struct gameState * state, int handPos){
	...
	//Add 2 actions to the game state
	state->numActions = 2;
	//BUG HERE
	//should be += 2
...
int greatHallCard(int player, struct gameState * state, int handPos){
	
	//Draw a card
	drawCard(player, state);

	//Increment the actions by one
	state->numActions += 1;
...
int embargoCard(int player, struct gameState * state, int handPos, int choice){
	
	//Add two coins to the game state
	state->coins = 2;
	//BUG HERE ^ should be += 2
	
	//If the selected supply pile is empty, return 1
	if(state->supplyCount[choice] == -1)
		return -1;
	
	//Increment embargo tokens on that supply pile
	state->embargoTokens[choice]++;
	
	//Discard the embargo card
	discardCard(handPos, player, state, 0);
